...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    for rollout_id in range(num_rollouts):
        ...
        if 'sim_transfer_cube' in task_name:
            BOX_POSE[0] = np.concatenate((sample_box_pose(),sample_box_pose(),sample_box_pose()))# used in sim reset
...
def forward_pass(data, policy):
    ...
    image_data, qpos_data, action_data, is_pad = image_data.to(torch.device("cpu")), qpos_data.to(torch.device("cpu")), action_data.to(torch.device("cpu")), is_pad.to(torch.device("cpu"))
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    policy_config = config['policy_config']
    resume_ckpt = None
    resume_ckpt = "/Users/is/VSCode/bachelors_diploma/v8_ALOHA_many_cubes_blue_v2_train_more/ckpt_dir/policy_epoch_2900_seed_0.ckpt"
